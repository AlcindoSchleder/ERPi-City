...
ORIGEM_ESCOLHAS = (
    ...
    (
        u'3',
        u'3 - Nacional - Mercadoria ou bem com Conteúdo de Importação ' +
        'superior a 40% e inferior ou igual a 70%.'
    ),
    (
        u'4',
        u'4 - Nacional - Cuja produção tenha sido feita em conformidade ' +
        'com os processos produtivos básicos de que tratam o Decreto-Lei ' +
        'nº 288/67, e as Leis nºs 8.248/91, 8.387/91, 10.176/01 e 11.484/ 07'
    ),
    (
        u'5',
        u'5 - Nacional - Mercadoria ou bem com Conteúdo de Importação inferior ' +
        'ou igual a 40% (quarenta por cento)'
    ),
    (
        u'6',
        u'6 - Estrangeira - Importação direta, sem similar nacional, constante ' +
        'em lista da Resolução CAMEX nº 79/2012 e gás natural'
    ),
    (
        u'7',
        u'7 - Estrangeira - Adquirida no mercado interno, sem similar nacional, ' +
        'constante em lista da Resolução CAMEX nº 79/2012 e gás natural'
    ),
    (
        u'8',
        u'8 - Nacional - Mercadoria ou bem com Conteúdo de Importação superior ' +
...
@receiver(post_save, sender=Categoria, dispatch_uid='after_save_categoria')
def after_save_categoria(instance, **kwargs):
    created = False if 'created' not in kwargs.keys() else kwargs['created']
    if not created:
        return
    pk = instance.pk
    fk = instance.fk_categorias_id if instance.fk_categorias_id is not None else pk
    if instance.class_code is None and instance.level == 0:
        instance.fk_categorias_id = fk
        if pk == fk:
            instance.class_code = str(pk).zfill(2)
            instance.level = 1
        else:
            # get parent record
            qry_cat = Categoria.objects.filter(id=fk)
            instance.class_code = qry_cat[0].class_code + '.' + str(pk).zfill(3)
            instance.level = instance.class_code.count('.') + 1
        instance.save()
